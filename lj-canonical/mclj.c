...
/**********************************************************************/
  ...
  for(i=0;i<=n-2;i++) {
    ...
    for(j=i;j<=n-1;j++) {
      ...
      if(r2<rc2) { 		//if the particles are near enough, they interact, otherwise not 
	rm6=1.0/(r2*r2*r2);
...
/**********************************************************************/
  ...
  if((ntprint>0)&&(nt%ntprint==0)) {
    ...
    printf(" %10d %12.5e %12.5e %12.5e\n",\
	   nt,(double)nacc/(n*ntskip),su,p); // nt .. number of executed jobs, nacc/(n*ntsikip) mean acceptance of moves, su energy per particle, p pressure per particle
...
FILE *fpo;
	int i;
        fpo=fopen("Paircorrf.txt","w");
	fprintf(fpo,"r	Paircorrelationfunction g(r)\n");
	for(i=0;i<n;i++) { 
			fprintf(fpo,"%8.5f	%8.5f\n",i*dr,ag[i]/(ntjob*2*3.141592*(i*dr)*(i*dr)*dr*rho*n));
	}
	fclose(fpo);
