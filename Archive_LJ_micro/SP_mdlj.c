...
/**********************************************************************/
  ...
  for(i=0;i<=n-2;i++) {
    for(j=i+1;j<=n-1;j++) {
      ...
      if(r2<rc2) {                   /* In range? */
	rm6=1.0/(r2*r2*r2);
	df=(24.0-48.0*rm6)*rm6/r2-(24.0-48.0/(rc2*rc2*rc2))/(rc2*rc2*rc2*rc2);
	fx[i]=fx[i]+df*dx;
...
/**********************************************************************/
  ...
  int i,j,k;
  double dx,dy,dz,r2,rm6,sk,su,sw,suc,swc,u;
  ...
  for(i=0;i<=n-2;i++) {
	...
	if(r2<rc2) {
	  rm6=1.0/(r2*r2*r2);
	  su=(4.0*rm6-4.0)*rm6;     /* Potential energy */
	  suc=(4.0/(rc2*rc2*rc2)-4.0)/(rc2*rc2*rc2);     /* Potential energy at rc*/
	  sw=sw+(24.0-48.0*rm6)*rm6;   /* Virial           */
	  swc=(24.0-48.0/(rc2*rc2*rc2))/(rc2*rc2*rc2)*(sqrt(r2)-sqrt(rc2));
	  u=u+su-suc-swc;
	}
	k=(int)(sqrt(r2)*drm1);
	if(k<=ndr-1) {
  ...
  if((ntprint>0)&&((nt/ntaskip)%ntprint==0)) {
    printf("%10d\t%12.5le\t%12.5le\t%12.5le\t%12.5le\t%12.5le\n",nt,\
	   2.0*(sk/n)/3.0,sk/n,su/n,u/n,rho*(2.0*sk-sw)/(3*n));
  }
